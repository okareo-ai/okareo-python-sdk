...
def test_voice_multiturn_deepgram(okareo: Okareo,  deepgram_voice_target: VoiceMultiturnTarget) -> None:
    run_voice_multiturn_test(okareo, deepgram_voice_target, "Deepgram")

def test_voice_multiturn_openai(okareo: Okareo, openai_voice_target: VoiceMultiturnTarget) -> None:
    run_voice_multiturn_test(okareo, openai_voice_target, "OpenAI")

def run_voice_multiturn_test(okareo: Okareo, voice_target: VoiceMultiturnTarget, vendor: str) -> None:
    ...
    scenario = okareo.create_scenario_set(
        ScenarioSetCreate(name=f"Product Returns — Conversation Context ({vendor})", seed_data=seed_data)
    )
