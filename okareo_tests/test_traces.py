    ...
    for attempt in range(max_attempts):
        try:
            ...
            response = requests.post(
                f"{base_url}/v0/find_spans_filter",
                headers=headers,
                json=payload
            )
...
def test_traces_endpoint_with_verification(api_key: Any, base_url: Any) -> Any:
                                ...
                                "attributes": [
                                    {
                                        ...
                                        "value": {"stringValue": "gemini-2.5-pro-exp-03-25"}
                                    },
                                    {
                                    ...
                                    {
                                        "key": "llm.messages",
                                        "value": {"stringValue": f"[{{\"content\": \"Test message with {random_string}\", \"role\": \"user\"}}]"}
                                    },
                                    {
                                    ...
                                    },
                                    {
                                        ...
                                        "value": {"stringValue": f"chatcmpl-{random_string}"}
                                    },
                                    {
                                    ...
                                    },
                                    {
                                        ...
                                        "value": {"stringValue": f"Test with {random_string}"}
                                    },
                                    {
                                        "key": "SpanAttributes.LLM_COMPLETIONS.0.content",
                                        "value": {"stringValue": f"Response to {random_string}"}
                                    }
                                ],
                                "status": {"code": 1},
                                "flags": 256
    ...
    assert verify_span_recorded(random_string, api_key, base_url), f"Span with random string '{random_string}' was not found in Okareo"
