...
def test_run_multiturn_run_test(rnd: str, okareo: Okareo) -> None:
    ...
    mut = okareo.register_model(
        name=rnd,
        model=MultiTurnDriver(
            driver_params={"driver_type": "openai", "max_turns": 2, "repeats": 1},
...
def test_run_multiturn_run_test_multiple_checks(rnd: str, okareo: Okareo) -> None:
    ...
    mut = okareo.register_model(
        name=rnd,
        model=MultiTurnDriver(
            driver_params={"driver_type": "openai", "max_turns": 2},
            target=OpenAIModel(
                model_id="gpt-4o-mini",
                temperature=1,
...
class CustomMultiturnModel(CustomTarget):
    def startConversation(self) -> ModelInvocation:
        session_id = "123"
        content = "Hello!"
        return ModelInvocation(content, "", {}, session_id)
    
    def continueConversation(self, conversation_id: str, messages: list) -> ModelInvocation | Any:
        content = "I can't help you with that."
        return ModelInvocation(content, "", {}, conversation_id)
...
def test_run_multiturn_custom_with_repeats(rnd: str, okareo: Okareo) -> None:
    ...
    scenario_set_create = ScenarioSetCreate(
        name=f"CustomMultiturnModel - {rnd}", seed_data=seeds
    )
    scenario = okareo.create_scenario_set(scenario_set_create)
    evaluation = model_under_test.run_test(
        name=f"CustomMultiturnModel - {random_string}",
    ...
    assert evaluation.name == f"CustomMultiturnModel - {random_string}"
