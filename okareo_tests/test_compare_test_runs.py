...
def test_compare_with_repeats(
    ...
    binary_names = {c["name"] for c in stats["binary_checks"]}
    assert "model_refusal" in binary_names, f"model_refusal not in {binary_names}"

    refusal_check = next(
        c for c in stats["binary_checks"] if c["name"] == "model_refusal"
    )
    ...
    numeric_names = {c["name"] for c in stats["numeric_checks"]}
    assert (
        "levenshtein_distance" in numeric_names
    ), f"levenshtein_distance not in {numeric_names}"

    lev_check = next(
        c for c in stats["numeric_checks"] if c["name"] == "levenshtein_distance"
    )
