...
_WEBRTC_REQUIRED_FIELDS = {
    ...
    "retell": ("agent_id",),
    # Daily has three server-side modes: direct (room_url), REST (the Daily
    # API key rides api_keys["voice"] at run time, no room_url -- Okareo
    # creates a room per conversation), and customer-brokered. The client
    # cannot see api_keys at target-validation time, so the server's
    # validate_required_fields is the authority; require nothing here.
    "daily": (),
