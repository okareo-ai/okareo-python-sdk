import logging
import os, io, json, time, wave, tempfile, base64, asyncio, requests, sys, uuid
...
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY"); assert OPENAI_API_KEY, "Set OPENAI_API_KEY"
...
def tts_pcm16(text: str, voice="echo", target_sr=API_SR) -> bytes:
    url = "https://api.openai.com/v1/audio/speech"
    headers = {"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": "gpt-4o-mini-tts", "voice": voice, "input": text, "response_format": "pcm"}
...
async def asr_openai_from_pcm16(pcm_bytes: bytes, sr: int, model: str = "gpt-4o-mini-transcribe") -> str:
...
class VoiceEdge(ABC):
    ...
    @abstractmethod
    async def send_pcm(self, pcm16: bytes, pace_realtime: bool = True, timeout_s: float = 20.0) -> tuple[bytes, Dict[str, Any]]:
        ...
...
class OpenAIRealtimeEdge(VoiceEdge):
    AUDIO_DONE_TYPES = {"response.output_audio.done", "response.audio.done", "output_audio.done"}
    ...
    async def connect(self, **kwargs) -> None:
        ...
        self.ws = await websockets.connect(self.ws_url, additional_headers=headers, max_size=None)
        ...
        await self.ws.send(json.dumps({
            ...
            "session": {
                ...
                "audio": {
                    "input": {
                        "format": {"type": "audio/pcm", "rate": self.edge_config.sr},
    ...
    async def send_pcm(self, pcm16: bytes, pace_realtime: bool = True, timeout_s: float = 30.0) -> tuple[bytes, Dict[str, Any]]:
        ...
        for ch in chunks:
            await self.ws.send(json.dumps({
                ...
                "audio": base64.b64encode(ch).decode("ascii")
            }))
            if pace_realtime:
        ...
        while True:
            ...
            elif t in self.AUDIO_DONE_TYPES:
                audio_finalized = True

            elif t in self.RESP_FINAL_TYPES:
                resp_finalized = True
...
class DeepgramRealtimeEdge(VoiceEdge):
    ...
    async def connect(self, **kwargs) -> None:
        self.ws = await websockets.connect(
            self.ws_url,
            subprotocols=["token", self.edge_config.api_key]
        )
        config_message = {
            "type": "Settings",
            "audio": {
        ...
        async def send_keep_alive(stop_event):
            try:
        ...
        self._keepalive_task = asyncio.create_task(send_keep_alive(self._keepalive_stop_event))
    ...
    def _handle_message_event(self, evt):
        etype = evt.get("type")
        if etype == "AgentAudioDone":
            self._agent_turn_complete.set()


    async def send_pcm(self, pcm16: bytes, pace_realtime: bool = True, timeout_s: float = 30.0, add_silence_ms: int = 400,) -> tuple[bytes, Dict[str, Any]]:
        assert self.ws and self._connected, "Call connect() first."
        self._audio_buf = bytearray()
        self._agent_turn_complete.clear()
...
class RealtimeClient:
    ...
    async def send_utterance(self, text: str, tts_voice: str = "echo", pace_realtime: bool = True, timeout_s: float = 60.0):
        ...
        agent_pcm, vendor_meta = await self.edge.send_pcm(user_pcm, pace_realtime=pace_realtime, timeout_s=timeout_s)

        # 3) Save agent WAV
        assistant_wav_path = None
        if agent_pcm:
            assistant_wav_path = self._store_wav(agent_pcm, prefix=f"agent_turn_{turn_id:03d}_")
...
class SessionManager:
    ...
    async def send(self, session_id: str, text: str, **send_kwargs):
